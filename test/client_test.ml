...
let test_broken_callback () =
  ...
  let m = ref 0 in
  Client.set_logger (fun s ->
      incr m;
      Printf.fprintf stderr "This error is not a failure: %s" s);
  let n = ref 0 in
  ...
  Client.immediate client (fun xs -> Client.watch xs "/tmp" "");

  Thread.delay 1.0;

  Client.immediate client (fun xs -> Client.write xs "/tmp" "foo");
...
let test_watch_callbacks () =
  ...
  let watch_callback _ =
    let domid = Client.immediate client (fun xs -> Client.read xs "domid") in
    finished := true;
  ...
  if not !finished then failwith "Test failed"
