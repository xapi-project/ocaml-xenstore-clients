...
let read_opt path xs =
  Lwt.catch
    (fun () -> read xs path >>= fun x -> Lwt.return (Some x))
...
module Device = struct
  ...
  let parse_backend_link x =
    ...
    | [ ""; "local"; "domain"; domid; "backend"; kind; _; devid ] -> (
        ...
        | Some domid, Some kind, Some devid -> Some { domid; kind; devid }
        | _, _, _ -> None)
    | _ -> None
            ...
            Lwt.catch
              (fun () ->
                immediate client (fun xs ->
                    ...
                    | Some b -> return (Some { backend = b; frontend })
                    | None -> return None))
              (fun _ -> return None))
          devids
        >|= to_list)
      kinds
    >>= fun ll -> return (List.concat ll)
  ...
  let backend_path_of_device (x : device) client =
    ...
    return
      (Printf.sprintf "%s/backend/%s/%u/%d" dom_path
  ...
  let backend_error_path_of_device (x : device) client =
    getdomainpath x.backend.domid client >>= fun dom_path ->
    return
      (Printf.sprintf "%s/error/backend/%s/%d" dom_path
         (string_of_kind x.backend.kind)
         x.frontend.domid)
  ...
  let frontend_path_of_device (x : device) client =
    getdomainpath x.backend.domid client >>= fun dom_path ->
    return
      (Printf.sprintf "%s/device/%s/%d" dom_path
  ...
  let frontend_error_path_of_device (x : device) client =
    ...
    return
      (Printf.sprintf "%s/error/device/%s/%d/error" dom_path
         (string_of_kind x.frontend.kind)
         x.frontend.devid)
  ...
  let hard_shutdown_request (x : device) client =
    ...
    immediate client (fun xs ->
        write xs online_path "0" >>= fun () -> rm xs frontend_path)
  ...
  let add device client =
    let backend_list = [] and frontend_list = [] and private_list = [] in
    ...
    transaction client (fun xs ->
        ...
        setperms xs hotplug_path
          Xs_protocol.ACL.
            { owner = device.backend.domid; other = NONE; acl = [] }
        >>= fun () ->
        Lwt_list.iter_s
          (fun (x, y) -> write xs (frontend_path ^ "/" ^ x) y)
          (("backend", backend_path) :: frontend_list)
        >>= fun () ->
        Lwt_list.iter_s
          (fun (x, y) -> write xs (backend_path ^ "/" ^ x) y)
          (("frontend", frontend_path) :: backend_list)
        >>= fun () ->
        mkdir xs private_data_path >>= fun () ->
        setperms xs private_data_path
          Xs_protocol.ACL.
            { owner = device.backend.domid; other = NONE; acl = [] }
        >>= fun () ->
        Lwt_list.iter_s
          (fun (x, y) -> write xs (private_data_path ^ "/" ^ x) y)
...
module Domain = struct
  let make domid client =
    ...
    let xsdata = [ ("xsdata", "xsdata") ] in
    let platformdata = [ ("platformdata", "platformdata") ] in
    let bios_strings = [ ("bios_strings", "bios_strings") ] in
    let roperm =
    ...
    transaction client (fun xs ->
        ...
        Lwt_list.iter_s
          (fun dir ->
            ...
            mkdir xs ent >>= fun () -> setperms xs ent roperm)
          [ "cpu"; "memory" ]
        >>= fun () ->
        ...
        Lwt_list.iter_s
          (fun dir ->
            let ent = Printf.sprintf "%s/%s" dom_path dir in
            mkdir xs ent >>= fun () -> setperms xs ent rwperm)
          [
            "device";
            "error";
            "drivers";
    ...
    immediate client (fun xs ->
        ...
        >>= fun () ->
        Lwt_list.iter_s
          (fun (x, y) -> write xs (dom_path ^ "/platform/" ^ x) y)
          platformdata
        >>= fun () ->
        Lwt_list.iter_s
          (fun (x, y) -> write xs (dom_path ^ "/bios-strings/" ^ x) y)
          bios_strings
        >>= fun () ->
  ...
  let shutdown domid req client =
    ...
    transaction client (fun xs ->
        ...
        if domain_exists then write xs path reason >>= fun () -> return true
        else return false)
  ...
  let destroy domid client =
    ...
    Lwt_list.iter_s
      (fun device -> Device.hard_shutdown device client)
      all_devices
    >>= fun () ->
    ...
    | Some vm_path ->
        immediate client (fun xs ->
            ...
            if List.filter (fun x -> x <> "") domains = [] then
              rm xs vm_path >>= fun () ->
              match vss_path with
    ...
    Lwt_list.iter_s
      (fun ty ->
        immediate client (fun xs ->
            rm xs (Printf.sprintf "%s/%s/%d" backend_path ty domid)))
      all_backend_types
end

let vm_shutdown domid client =
  Domain.shutdown domid () client >>= fun _ -> Domain.destroy domid client
...
let vm_cycle domid client =
  vm_start domid client >>= fun () -> vm_shutdown domid client

let rec between start finish =
  if start > finish then [] else start :: between (start + 1) finish

let sequential n client : unit Lwt.t =
  Lwt_list.iter_s (fun domid -> vm_cycle domid client) (between 0 n)

let parallel n client =
  Lwt_list.iter_p (fun domid -> vm_cycle domid client) (between 0 n)

let query m n client =
  Lwt_list.iter_s (fun domid -> vm_start domid client) (between 0 n)
  >>= fun () ->
  let rec loop i =
    if i = m then Lwt.return_unit
    else
      Lwt_list.iter_p
        (fun domid ->
          immediate client (fun xs ->
  ...
  in
  loop 0 >>= fun () ->
  Lwt_list.iter_s (fun domid -> vm_shutdown domid client) (between 0 n)

let time f =
  let start = Unix.gettimeofday () in
  f () >>= fun () -> return (Unix.gettimeofday () -. start)
...
let main () =
  ...
  if List.mem "-h" args then (
    ...
    Lwt_io.write Lwt_io.stdout
      (Printf.sprintf "%d sequential starts and shutdowns: %.02f\n" n t)
    >>= fun () ->
    time (fun () -> parallel n client) >>= fun t ->
    Lwt_io.write Lwt_io.stdout
      (Printf.sprintf "%d parallel starts and shutdowns: %.02f\n" n t)
    >>= fun () ->
    time (fun () -> query 1000 n client) >>= fun t ->
    Lwt_io.write Lwt_io.stdout
      (Printf.sprintf "%d read queries per %d VMs: %.02f\n" 1000 n t))
