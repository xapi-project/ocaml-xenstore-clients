...
let parse_expr s =
  ...
  let flatten s =
    ...
    in
    let ident is =
      if is = [] then [] else [ Ident (String.concat "" (List.rev is)) ]
    in
    let is, tokens =
      List.fold_left
        (fun (is, tokens) x ->
        ...
        ([], []) (to_list s)
    in
    ident is @ tokens |> List.rev |> Stream.of_list
  in
  ...
  let rec parse_atom (__strm : _ Stream.t) =
    ...
    | Some (Kwd "not") ->
        Stream.junk __strm;
        let e =
          try parse_expr __strm with Stream.Failure -> raise (Stream.Error "")
        in
        Not e
    | Some (Kwd "(") -> (
        Stream.junk __strm;
...
let rec eval_expression expr xs =
  match expr with
  | Val path ->
      Lwt.catch
        (fun () -> read xs path >>= fun _k -> return true)
        (function Enoent _ -> return false | e -> Lwt.fail e)
  | Not a -> eval_expression a xs >>= fun a' -> return (not a')
  | And (a, b) ->
      eval_expression a xs >>= fun a' ->
      eval_expression b xs >>= fun b' -> return (a' && b')
  | Or (a, b) ->
      eval_expression a xs >>= fun a' ->
      eval_expression b xs >>= fun b' -> return (a' || b')
  | Eq (Val path, Val v) ->
      Lwt.catch
        (fun () -> read xs path >>= fun v' -> return (v = v'))
        (function Enoent _ -> return false | e -> Lwt.fail e)
...
let usage () =
  ...
  let lines =
    [
      ...
      "   -- block until either key \"/foo\" is created or key \"/bar\" has \
       value \"hello\"";
    ]
  in
          ...
          (function
            | Invalid_expression as e ->
                Lwt_io.write Lwt_io.stderr
                  "Invalid expression; expected <key=val> [and key=val]*\n"
                >>= fun () -> Lwt.fail e
            | e -> Lwt.fail e)
        >>= fun items ->
        make () >>= fun client ->
